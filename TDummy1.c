...
void start_t(){ //Initialise the period
	...
	time_value.it_interval.tv_sec = 2;
...
void Start_Task(){
	struct timespec tstart, tend; //Initialise the structure for the timer
		...
		delay(0.6); //Use 0.6 execution time for the first task
		clock_gettime(CLOCK_MONOTONIC, &tend); //End task and print the results
		int64_t c = ((int64_t)tend.tv_sec * 1000000000 + (int64_t)tend.tv_nsec) - ((int64_t)tstart.tv_sec * 1000000000 + (int64_t)tstart.tv_nsec);
		printf("|Task 1\tRun for: %.1fs|\t|Sum of 10 and 2 is %.1f |\n", c, (c * 0.000000001), result);
}


void timer_handler(){ //This is the timer handler to get the start signal and send end signal
	printf("|Initiate Task 1... PID: %d|\n", getpid());
	Start_Task(); //Jump to Start_Task
	printf("|Task 1 Execution Done...|\n");
...
int main(){
	...
	set_param.sched_priority = 30; //Priority of the task is T1>T2/T4>T3 I set the the priority as 30, 20, 10, 20
